...
import com.wang.wangaicodegenerator.core.handler.StreamHandlerExecutor;
...
public class AppServiceImpl extends ServiceImpl<AppMapper, App> implements AppService {
    ...
    public String deployApp(Long appId, User loginUser) {
        ...
        if (codeGenTypeEnum == CodeGenTypeEnum.VUE_PROJECT) {
            ...
            ThrowUtils.throwIf(!buildProject, ErrorCode.SYSTEM_ERROR, "构建 Vue 项目失败,请检查代码和依赖");
            //检查dist目录是否存在
            File distDir = new File(sourceDirPath, "dist");
            ThrowUtils.throwIf(!distDir.exists(), ErrorCode.SYSTEM_ERROR, "构建 Vue 项目完成但未生成dist目录");
            //将dist目录作为部署源
            sourceDir = distDir;
