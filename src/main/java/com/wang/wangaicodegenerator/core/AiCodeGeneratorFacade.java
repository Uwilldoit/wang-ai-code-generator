...
import com.wang.wangaicodegenerator.ai.model.message.AiResponseMessage;
import com.wang.wangaicodegenerator.ai.model.message.ToolExecutedMessage;
import com.wang.wangaicodegenerator.ai.model.message.ToolRequestMessage;
    ...
    public File generateAndSaveCode(String userMessage, CodeGenTypeEnum codeGenTypeEnum,Long appId) {
        ...
        return switch (codeGenTypeEnum) {
            case HTML -> {
                ...
                yield CodeFileSaverExecutor.execute(result, CodeGenTypeEnum.HTML,appId);
            }
            case MULTI_FILE -> {
                MultiFileCodeResult result = aiCodeGeneratorService.generateMultiFileCode(userMessage);
                yield CodeFileSaverExecutor.execute(result, CodeGenTypeEnum.MULTI_FILE,appId);
            }
            default -> {
    ...
    private Flux<String> processCodeStream(Flux<String> codeStream, CodeGenTypeEnum codeGenType,Long appId) {
        ...
        return codeStream.doOnNext(chunk -> {
            ...
            try {
                ...
                // 使用执行器保存代码
                File saveDir = CodeFileSaverExecutor.execute(parsedResult, codeGenType,appId);
